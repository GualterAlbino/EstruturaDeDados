...
int TabelaHash::quadratico (int indice,int i) // a estrategia de colisao quadratico consiste em pegar o indice "indice" + uma constante * tentativas "i" + uma sesunda
{                                             //constate * tenstativa ao quadrado (i*1) e devolver um novo indice onde sera feita uma nova tentativa.

    indice = (indice + 2*i + 5*i*i) %tam ;
    return indice ;
...
bool TabelaHash::insereHash(int chave)
{
    ...
    {
        nindice = quadratico(indice,i); // untiliza a funçao quadratico para calcular um novo indice.
        i ++;
